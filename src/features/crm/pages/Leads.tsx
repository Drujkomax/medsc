...
const Leads = () => {
  ...
  const getStageLabel = (stage: string) => {
    const stageMap: Record<string, string> = {
      ...
      proposal: t("stages.proposal", "Отправил КП"),
      negotiation: t("stages.negotiation", "Переговоры"),
      closed: t("stages.closed", "Успешно"),
      lost: t("stages.lost", "Отказ"),
    };
    ...
    try {
      ...
      toast({
        ...
        description: assigneeId
          ? t("leads.leadAssigned", "Лид назначен")
          : t("leads.leadUnassigned", "Назначение лида снято"),
      });
    } catch (error) {
  ...
  const leadStages = [
    {
      value: "new",
      label: t("stages.new", "Новый"),
      count: 0,
      color: "bg-blue-500",
    },
    {
      value: "contacted",
      label: t("stages.contacted", "Связались"),
      count: 0,
      color: "bg-yellow-500",
    },
    {
      value: "qualified",
      label: t("stages.qualified", "Квалифицирован"),
      count: 0,
      color: "bg-purple-500",
    },
    {
      value: "proposal",
      label: t("stages.proposal", "Отправил КП"),
      count: 0,
      color: "bg-orange-500",
    },
    {
      value: "negotiation",
      label: t("stages.negotiation", "Переговоры"),
      count: 0,
      color: "bg-indigo-500",
    },
    {
      value: "closed",
      label: t("stages.closed", "Успешно"),
      count: 0,
      color: "bg-green-500",
    },
    {
      value: "lost",
      label: t("stages.lost", "Отказ"),
      count: 0,
      color: "bg-red-500",
    },
  ];
  ...
  const handleArchiveLead = async (id: string) => {
    try {
      ...
      toast({
        title: t("common.success", "Успешно"),
        description: t("leads.leadArchived", "Лид архивирован"),
      });
    } catch (error) {
      toast({
        title: t("common.error", "Ошибка"),
        description: t("leads.archiveError", "Ошибка при архивировании лида"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleStageChange = async (leadId: string, newStage: string) => {
    try {
      ...
      toast({
        title: t("common.success", "Успешно"),
        description: t("leads.stageUpdated", "Статус лида обновлен"),
      });
    } catch (error) {
      console.error("Stage change error:", error);
      toast({
        title: t("common.error", "Ошибка"),
        description: `${t("leads.stageUpdateError", "Ошибка при обновлении статуса:")} ${
          error instanceof Error ? error.message : t("leads.unknownError", "Неизвестная ошибка")
        }`,
        variant: "destructive",
      });
    }
  ...
  const handleDealSuccess = () => {
    ...
    toast({
      title: t("common.success", "Успешно"),
      description: t("leads.dealCreatedRedirect", "Сделка создана. Переходим к разделу сделок..."),
    });
  ...
  const leadCounts = leadStages.reduce(
    (acc, stage) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-2 md:gap-4">
        {/* Total Leads Card - for all roles */}
            ...
            <div className="flex gap-2">
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {startDate ? format(startDate, "dd.MM.yyyy") : t("leads.from", "От")}
                  </Button>
                </PopoverTrigger>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {endDate ? format(endDate, "dd.MM.yyyy") : t("leads.to", "До")}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                ...
                <TableRow>
                  {canBulkActions && (
                    <TableHead className="w-12">
                      <Checkbox
                        checked={
                          selectedLeadIds.length === filteredAndSortedLeads.length && filteredAndSortedLeads.length > 0
                        }
                  ...
                  <TableHead>{t("leads.status", "Статус")}</TableHead>
                  <TableHead>{t("leads.quality", "Качество")}</TableHead>
                  <TableHead
              ...
              <TableBody>
                {filteredAndSortedLeads.map((lead) => (
                  <TableRow
                    ...
                    className="cursor-pointer hover:bg-muted/50"
                    onClick={(e) => {
                      // Check that click is not on dropdown menu or buttons or checkboxes
